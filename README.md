...
Если у нас 1000 машин и нужно много вычислений на огромных объёмах данных, то главный критерий выбора СУБД — не хранение, а обработка. То есть нам важнее не «где лежат данные», а как быстро и эффективно их обработать параллельно.
Основной критерий:
Поддержка распределённых вычислений «из коробки» + масштабируемость без боли.

Исходя из этого, классические реляционные СУБД (типа PostgreSQL или MySQL) не подойдут — они плохо масштабируются горизонтально для аналитических нагрузок.

Лучший выбор: СУБД/платформа на основе модели MapReduce или, ещё лучше, DAG-вычислений
А именно — Apache Spark (часто поверх HDFS или S3) или СУБД, построенные на похожей архитектуре, например:

ClickHouse (для аналитики в реальном времени),
Apache Druid (для быстрых агрегаций),
Google BigQuery / Snowflake (если в облаке),
Но при 1000 своих машин — скорее всего, Spark + Parquet/ORC в HDFS.
Почему Spark — король здесь?
Он использует модель вычислений на основе DAG (Directed Acyclic Graph) — это умнее и быстрее, чем старый добрый MapReduce (как в Hadoop).
Данные обрабатываются в памяти, а не пишутся на диск после каждого шага → в разы быстрее.
Легко распределяет задачи по 1000 узлам, сам управляет отказоустойчивостью.
Поддерживает SQL (через Spark SQL), Python, Scala — удобно для аналитиков и инженеров.
А как же СУБД?
В такой архитектуре «СУБД» часто превращается в распределённое хранилище + вычислительный движок.
Например:

ClickHouse может работать в кластере и сам делать агрегации на каждом шарде.
Но если задачи сложные (ML, ETL, джойны по терабайтам), то Spark гибче.
Вывод:
При 1000 машинах и тяжёлых вычислениях:

Выбираем не «СУБД в классическом понимании», а распределённую вычислительную платформу.
Лучшая модель — DAG (как в Apache Spark), потому что она быстрая, гибкая и масштабируется линейно.
Хранилище — HDFS, S3 или распределённая файловая система, данные в колоночном формате (Parquet).
А SQL-интерфейс можно добавить сверху (Spark SQL, Presto, Trino) — и всё будет «как СУБД», но мощнее.
Так что да — когда у тебя целая ферма из 1000 серверов, думай не о таблицах, а о параллельных вычислениях